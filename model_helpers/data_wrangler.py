...
def data_wrangler(file_name, look_back=LOOK_BACK, transform_data = True, unseen=False):
    ...
    df = prep_data[['Open', 'High', 'Low', 'Close', 'Volume']]
    # Replace commas in Volume label
    df.loc[:, 'Volume'] = df['Volume'].str.replace(',', '').astype('float64')
    ...
    if(transform_data):
        ...
        df.loc[:, ['Open', 'High', 'Low', 'Close', 'Volume']] = scaler.fit_transform(df[['Open', 'High', 'Low', 'Close', 'Volume']]).astype('float64')
    
    # Numpy ndarray
    data = df[['Open', 'High', 'Low', 'Close', 'Volume']].astype('float64')
    ...
    # Using the first and last indices of the dataframe to select items
    X = data.loc[: data.index[-1]-1, ['Open', 'High', 'Low', 'Volume']]
    # y = pd.Series(data.loc[data.index[0]+1:, 'Close'])
    y = data.loc[data.index[0]+1:, 'Close']
