...
EPOCHS = 200
MAX_EPOCHS = 2 # max_epochs = 8 or 10 seems good for large or small sample
DISABLE_RESUME = True
look_back = 7
...
class CustomHyperModel(kt.HyperModel):
    ...
    def build(self, hp):
        ...
        # optimizers = hp.Choice('optimizer', values =('SGD', 'RMSprop', 'Adam', 'Adadelta', 'Adagrad', 'Adamax', 'Nadam', 'Ftri'))
        # loss = hp.Choice('loss', values=('mean_squared_error', 'mean_absolute_error', 'root_mean_squared_error', 'huber'))
        ...
        #     case 'Ftri': optimizer = keras.optimizers.Ftri(learning_rate=learning_rate)
...
stop_early = keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0.0001, patience=5)
...
# history = model.fit(X_train, y_train, epochs=MAX_EPOCHS, validation_data=(X_val, y_val), callbacks=[stop_early])
...
future_steps = 56  # Number of days to predict
